...
@Component
public class SoapRequestAuthFilter extends OncePerRequestFilter {
  ...
  private static final Logger EVENT_LOGGER = LoggerFactory.getLogger(SoapRequestAuthFilter.class);
  ...
  public void doFilterInternal(final HttpServletRequest request, final HttpServletResponse response, final FilterChain chain)
    ...
    if (roles.isEmpty()) {
      EVENT_LOGGER.error(ROLES_ARE_EMPTY);
    ...
    try {
      ...
      EVENT_LOGGER.error("Error in SoapRequestAuthFilter ", e);
